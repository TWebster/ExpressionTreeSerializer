...
namespace ExpressionSerialization
{
	public class DynamicQueryProvider : QueryProvider
	{
		...
		dynamic DelegateExpressionCallToProvider(IQueryProvider provider, Expression e, Type TResult)
		{			
		...
		/// <returns></returns>
		dynamic GetIEnumerableOf(Type elementType)
		{
			Type listType = typeof(List<>).MakeGenericType(elementType);			
			dynamic list = CreateInstance(listType);
			for (int i = 0; i < 10; i++)
			{
				dynamic instance = CreateInstance(elementType);
				list.Add(instance);
			}
			return list;
		}
