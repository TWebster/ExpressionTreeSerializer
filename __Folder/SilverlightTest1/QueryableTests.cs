...
namespace UnitTests
{
	...
	public class QueryableTests : BaseTests
	{
		...
		{
			IQueryProvider clientprovider = new ClientQueryProvider();
		...
		public void IQueryProvider_CreateQuery2()
		{
			...
			//This next statement WILL call IQueryProvider.CreateQuery.
			//Expression is MethodCallExpression:
			IQueryable<Customer> query = new Query<Customer>(new ClientQueryProvider());
			query = from c in query
					where c.ID != null
					select c;
		...
		public void QueryExpressionXmlConverterTest()
		{
			...
			Query<Customer> query = new Query<Customer>(new ClientQueryProvider());
