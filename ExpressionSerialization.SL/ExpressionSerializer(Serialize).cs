...
namespace ExpressionSerialization
{
    public partial class ExpressionSerializer
    {
        ...
        public ExpressionSerializer(TypeResolver resolver)
        {
        ...
        private object GenerateXmlFromProperty(Type propType, string propName, object value)
        {
			...
			if (this.resolver.knownTypes.Contains(value.GetType()))//this.resolver.knownTypes.Contains(value.GetType()))
			{
				//expected: caller invokes with value == a ConstantExpression.Value
				return GenerateXmlFromKnownTypes(propName, instance: value);
			}
		...
		private bool IsIEnumerableOf(Type enumerableType, Type elementType)
		{
			...
			if (elementType.IsAssignableFrom(typeArgs[0]))
				return false;
			if (!typeof(IEnumerable<>).MakeGenericType(typeArgs).IsAssignableFrom(elementType))
				return false;
			return true;
		}
