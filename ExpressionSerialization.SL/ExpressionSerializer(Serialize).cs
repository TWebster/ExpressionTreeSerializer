...
namespace ExpressionSerialization
{
    public partial class ExpressionSerializer
    {
        ...
        private object GenerateXmlFromProperty(Type propType, string propName, object value)
        {
			...
			{
				Type knownType;
				if (this.resolver.HasMappedKnownType(value.GetType(), out knownType))//this.resolver.knownTypes.Contains(value.GetType()))
				{					
					return GenerateXmlFromKnownTypes(propName, instance: value, knownType: knownType);
				}
				else
