	...
	public partial class ExpressionSerializer
	{
	   ...
	   */
		...
		private object ParseConstantFromElement(XElement xml, string elemName, Type expectedType)
		{
			...
			if (resolver.knownTypes.Contains(expectedType))
			{
				DataContractSerializer serializer = new DataContractSerializer(expectedType, this.resolver.knownTypes);
				XElement value = xml.Element("Value");
				string xmlvalue = value.Value;
